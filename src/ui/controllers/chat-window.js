...
class ChatWindowUI {
    ...
    setupEventListeners() {
        ...
        if (window.emptyAPI) {
            ...
            window.emptyAPI.onTranscriptionReceived((event, data) => {
                if (data && data.text) {
            ...
            if (window.emptyAPI.onInterimTranscription) {
                window.emptyAPI.onInterimTranscription((event, data) => {
                    if (data && data.text) {
            ...
            window.emptyAPI.onSpeechStatus((event, data) => {
                if (data && data.status) {
            ...
            window.emptyAPI.onSpeechError((event, data) => {
                if (data && data.error) {
            ...
            // Session handlers
            window.emptyAPI.onSessionCleared(() => {
            ...
            window.emptyAPI.onOcrError((event, data) => {
                this.addMessage(`OCR Error: ${data.error}`, 'error');
            });
            
            window.emptyAPI.onLlmResponse((event, data) => {
            ...
            window.emptyAPI.onLlmError((event, data) => {
                this.addMessage(`LLM Error: ${data.error}`, 'error');
            });
            
            window.emptyAPI.onTranscriptionLlmResponse((event, data) => {
                if (data && data.response) {
    ...
    setupUIHandlers() {
        ...
        this.elements.micButton.addEventListener('click', async () => {
            ...
            try {
                if (this.isRecording) {
                    await window.emptyAPI.stopSpeechRecognition();
                } else {
                    await window.emptyAPI.startSpeechRecognition();
                }
            } catch (error) {
    ...
    async handleSkillActivated(skillName) {
        try {
            ...
            const skillPrompt = await window.emptyAPI.getSkillPrompt(skillName);
    ...
    async sendMessage() {
        ...
        if (text) {
            ...
            try {
                if (window.emptyAPI && window.emptyAPI.sendChatMessage) {
                    await window.emptyAPI.sendChatMessage(text);
                }
            } catch (error) {
