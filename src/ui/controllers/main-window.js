...
class MainWindowUI {
    ...
    async loadCurrentInteractionState() {
        try {
            ...
            if (window.emptyAPI && window.emptyAPI.getWindowStats) {
                const stats = await window.emptyAPI.getWindowStats();
    ...
    async loadSpeechAvailability() {
        try {
            if (window.emptyAPI && window.emptyAPI.getSpeechAvailability) {
                this.speechAvailable = await window.emptyAPI.getSpeechAvailability();
    ...
    resizeWindowToContent() {
        ...
        setTimeout(() => {
            ...
            if (commandTab && window.emptyAPI && window.emptyAPI.resizeWindow) {
                ...
                window.emptyAPI.resizeWindow(width, height);
            }
        }, 100);
    }
    ...
    if (!this.statusDot || !this.skillIndicator || !this.micButton || !this.screenshotButton) {
        ...
        this.screenshotButton.addEventListener('click', () => {
            if (this.isInteractive && window.emptyAPI && window.emptyAPI.takeScreenshot) {
                window.emptyAPI.takeScreenshot();
            }
        });
        ...
        this.skillIndicator.addEventListener('click', () => {
            ...
            const newSkill = 'dsa';
            if (window.emptyAPI && window.emptyAPI.updateActiveSkill) {
                window.emptyAPI.updateActiveSkill(newSkill).then(() => {
        ...
        this.micButton.addEventListener('click', () => {
            if (this.isInteractive) {
                if (this.isRecording) {
                    window.emptyAPI.stopSpeechRecognition();
                } else {
                    window.emptyAPI.startSpeechRecognition();
                }
            }
        });
        ...
        if (this.languageSelect) {
            ...
            this.languageSelect.addEventListener('change', (e) => {
                ...
                setTimeout(() => {
                    ...
                    if (commandTab && window.emptyAPI && window.emptyAPI.resizeWindow) {
                        const rect = commandTab.getBoundingClientRect();
                        window.emptyAPI.resizeWindow(Math.ceil(rect.width), Math.ceil(rect.height));
                    }
                }, 50);
            });
        }
    ...
    setupEventListeners() {
        if (window.emptyAPI) {
            ...
            window.emptyAPI.onInteractionModeChanged((event, interactive) => {
                logger.debug('Interaction mode changed received:', interactive);
                this.handleInteractionModeChanged(interactive);
            });

            window.emptyAPI.onRecordingStarted(() => {
                this.handleRecordingStarted();
            });

            window.emptyAPI.onRecordingStopped(() => {
                this.handleRecordingStopped();
            });

            window.emptyAPI.onSkillChanged((event, data) => {
                if (data && data.skill) {
                    this.handleSkillChanged(data);
                }
            });

            window.emptyAPI.onSpeechAvailability((event, data) => {
    ...
    moveWindow(direction) {
        ...
        if (window.emptyAPI && window.emptyAPI.moveWindow) {
            ...
            window.emptyAPI.moveWindow(deltaX, deltaY);
    ...
    async showGeminiConfig() {
        try {
            const status = await window.emptyAPI.getGeminiStatus();
    ...
    async configureGemini() {
        ...
        try {
            const result = await window.emptyAPI.setGeminiApiKey(apiKey);
            if (result.success) {
    ...
    async testGeminiConnection() {
        try {
            const result = await window.emptyAPI.testGeminiConnection();
            if (result.success) {
    ...
    openSettings() {
        try {
            if (window.emptyAPI && window.emptyAPI.showSettings) {
                window.emptyAPI.showSettings();
            } else {
                logger.error('emptyAPI or showSettings not available');
                return;
            }
    ...
    showSettingsMenu() {
        ...
        const quitOption = this.createMenuItem('Quit EmptyButHD', 'fa-power-off', () => {
            if (window.emptyAPI) {
                window.emptyAPI.quitApp();
            }
        });
