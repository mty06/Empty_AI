...
class WindowManager {
  constructor() {
    ...
    this.windowConfigs = {
      main: {
        ...
        height: 35,
        useContentSize: true,
        file: 'src/ui/views/index.html',
        title: 'OpenCluely'
      },
      chat: {
        width: 500,
        height: 700,
        file: 'src/ui/views/chat.html',
        title: 'Chat'
      },
      llmResponse: {
        width: 840,
        height: 480,
        file: 'src/ui/views/llm-response.html',
        title: 'AI Response',
        alwaysOnTop: true
      },
      settings: {
  ...
  async createMainWindow() {
    ...
    setTimeout(() => {
      logger.info('About to call showOnCurrentDesktop for main window');
  ...
  logger.info('BrowserWindow created', {
    ...
    config: {
      width: windowConfig.width,
      height: windowConfig.height,
      file: windowConfig.file
    }
  });

  // Listen for renderer process errors
  window.webContents.on('crashed', () => {
    logger.error('Window renderer process crashed', { type, windowId: window.id });
  });

  window.webContents.on('unresponsive', () => {
    logger.warn('Window became unresponsive', { type, windowId: window.id });
  });

  // Load the HTML file
    try {
      logger.info('Loading HTML file', { type, file: windowConfig.file });
      await window.loadFile(windowConfig.file);
      logger.info('HTML file loaded successfully', { type, file: windowConfig.file });
    } catch (error) {
  ...
  showOnCurrentDesktop(win) {
    ...
    logger.info('showOnCurrentDesktop called', {
      windowId: win.id,
      title: win.getTitle(),
